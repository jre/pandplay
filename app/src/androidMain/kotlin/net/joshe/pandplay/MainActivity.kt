...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        libraryViewModel.playlistTitleChanged.observe(this) { title ->
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU &&
            ...
            registerForActivityResult(ActivityResultContracts.RequestPermission()) { granted ->
                Log.i("MAIN", "requested notification permission: $granted")
    ...
    override fun onStart() {
        Log.v("MAIN", "onStart")
        super.onStart()
        lifecycleScope.launch { libraryViewModel.startBrowser(this@MainActivity) }
    }
    ...
    override fun onCreateOptionsMenu(menu: Menu?): Boolean {
        ...
        if (menu is MenuBuilder)
            @Suppress("UsePropertyAccessSyntax") // the property is apparently package-private?!?
    ...
    override fun onPrepareOptionsMenu(menu: Menu): Boolean {
        ...
        Log.v("MAIN", "building options menu with ${stations.size} stations, ${ids.size} cached ids and selecting ${selected?.stationName}")
