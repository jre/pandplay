...
private const val workNameNow = "$PANDPLAY_CONST_ID.DownloadWorker.downloadSongsImmediately"
private const val workNamePeriod = "$PANDPLAY_CONST_ID.DownloadWorker.downloadSongsPeriodically"
private const val inputDataManual = "$PANDPLAY_CONST_ID.DownloadWorker.inputDataManualWork"
...
fun downloadSongsPeriodically(context: Context) {
    ...
    val constraints = Constraints.Builder()
        ...
        .setRequiresDeviceIdle(PrefKey.DL_REQUIRE_IDLE.getBoolWithDefault(context))
...
class DownloadWorker(context: Context, params: WorkerParameters): CoroutineWorker(context, params) {
    ...
    private suspend fun pruneSongs(station: Station, added: MutableMap<SongId, Long>, sizes: MutableMap<SongId, Long>, quotaMB: Long) {
        ...
        Log.v("WORKER", "")
        while (chronological.isNotEmpty() && totalBytes > quotaMB * 1024 * 1024) {
            val dead = chronological.removeAt(0) // JFC it is idiotic that I can't use .removeFirst() here
